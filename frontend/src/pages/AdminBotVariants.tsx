...
import { botVariantsApi, skillsApi, botsApi } from "../lib/api";
import { Plus, X, Edit2, Trash2, Bot, Eye } from "lucide-react";
...
export default function AdminBotVariants() {
  ...
  const [panelMode, setPanelMode] = useState<"view" | "edit" | "preview">("view");
  ...
  const [previewSkillNames, setPreviewSkillNames] = useState<string[]>([]);
  ...
  const openVariant = (v: BotVariant) => {
    ...
    setPreviewSkillNames([]);
  };

  const handlePreviewPrompt = async (variantId: number) => {
    setLoadingPreview(true);
    setPreviewPrompt(null);
    setPreviewSkillNames([]);
    setPanelMode("preview");
    try {
      const res = await botsApi.previewPrompt(variantId);
      const data = res.data;
      setPreviewPrompt(typeof data === "string" ? data : (data.prompt ?? data.assembled_prompt ?? JSON.stringify(data, null, 2)));
      if (Array.isArray(data.skills_included)) {
        setPreviewSkillNames(data.skills_included.map((s: { name?: string }) => s.name ?? String(s)));
      }
    } catch {
          ...
          <div className="flex border-b border-gray-100 px-5">
            {(["view", "edit", "preview"] as const).map((tab) => (
              <button
                key={tab}
                onClick={() => {
                  if (tab === "preview" && selectedVariant) {
                    handlePreviewPrompt(selectedVariant.id);
                  } else {
                    setPanelMode(tab);
                  }
                }}
              ...
              >
                {tab === "view" ? "Chi tiết" : tab === "edit" ? "Chỉnh sửa" : "Preview Prompt"}
              </button>
            ))}
          </div>
            ...
            {panelMode === "preview" ? (
              <div className="space-y-3">
                <div className="flex items-center gap-2 mb-3">
                  <Eye className="w-4 h-4" style={{ color: "#028a39" }} />
                  <h3 className="text-sm font-semibold text-gray-800">Assembled System Prompt</h3>
                </div>
                {loadingPreview ? (
                  ...
                  <>
                    {previewSkillNames.length > 0 && (
                      <div className="mb-3">
                        <p className="text-xs font-semibold text-gray-600 mb-1">Skills included ({previewSkillNames.length})</p>
                        <div className="flex gap-1 flex-wrap">
                          {previewSkillNames.map((name) => (
                            <span key={name} className="px-2 py-0.5 bg-green-50 text-green-700 rounded text-xs">
                              {name}
                            </span>
                          ))}
                        </div>
                    ...
                    <pre className="bg-gray-50 rounded-lg p-3 text-xs font-mono text-gray-700 overflow-x-auto whitespace-pre-wrap max-h-[60vh] overflow-y-auto">
                      {previewPrompt ?? "(Không có nội dung)"}
                    </pre>
                  </>
                )}
              </div>
